...
const CalendarCard = ({ calendar, onEdit, onDelete }) => {
  ...
  const stats = useMemo(() => {
    const holidays = calendar.months.reduce(
      (count, month) => count + (month.holidays?.length || 0),
      0,
    );
    const overrides = calendar.months.reduce(
      (count, month) => count + (month.workingDaysOverride?.length || 0),
      0,
    );
  ...
  const monthModels = useMemo(() => {
    const weeklyOffSet = new Set(
      (calendar.weeklyOff || []).map((day) => String(day).trim().toUpperCase()),
    );
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Weekly off: {calendar.weeklyOff.join(", ") || "Not configured"}
          </p>
        </div>
      ...
      >
        ...
        {expanded ? "Hide Full Year Calendar" : "View Full Year Calendar"}        {expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
      </button>
                ...
                {month.cells.map((cell) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {cell.holidayTitle ? (
                        <p className="truncate text-[10px] text-rose-700">
                          {cell.holidayTitle}
                        </p>
                      ) : null}
